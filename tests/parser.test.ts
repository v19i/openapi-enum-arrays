...
export type NonEnum = string | number;
      ...
      expect(
        result.some((e) => e.values.join(",") === "active,inactive,pending"),
      ).toBe(true);
      expect(result.some((e) => e.values.join(",") === "red,blue,green")).toBe(
        true,
      );

      // Test that original paths are preserved
      expect(
        result.some((e) => e.originalTypePath === "export type Status"),
      ).toBe(true);
      expect(
        result.some((e) => e.originalTypePath === "export type Color"),
      ).toBe(true);
    });
...
export type ApiData = {
  ...
  nonEnum: string;
      ...
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "success,error,loading" &&
            e.originalTypePath === "ApiData.status",
        ),
      ).toBe(true);
      ...
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "high,medium,low" &&
            e.originalTypePath === "ApiData.priority",
        ),
      ).toBe(true);
    });
...
export type NestedTestData = {
    ...
    config?: {
      ...
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "dev,prod" &&
            e.originalTypePath === "NestedTestData.mode",
        ),
      ).toBe(true);
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "tag1,tag2,tag3" &&
            e.originalTypePath === "NestedTestData.query.tags",
        ),
      ).toBe(true);
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "name,popularity" &&
            e.originalTypePath === "NestedTestData.query.sort",
        ),
      ).toBe(true);
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "active,inactive" &&
            e.originalTypePath === "NestedTestData.query.status",
        ),
      ).toBe(true);
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "mysql,postgres,sqlite" &&
            e.originalTypePath === "NestedTestData.config.database.type",
        ),
      ).toBe(true);
    });
...
export type GetApiResourcesData = {
    ...
    url: '/api/resources/{resourceId}';
      ...
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "name,createdAt,updatedAt" &&
            e.originalTypePath === "GetApiResourcesData.query.sortBy",
        ),
      ).toBe(true);
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "asc,desc" &&
            e.originalTypePath === "GetApiResourcesData.query.sortOrder",
        ),
      ).toBe(true);
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "active,inactive,pending" &&
            e.originalTypePath === "GetApiResourcesData.query.status",
        ),
      ).toBe(true);
    });
    ...
    query?: {
      ...
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "asc,desc" &&
            e.originalTypePath === "ApiWithAnnotations.query.sortOrder",
        ),
      ).toBe(true);
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "active,inactive" &&
            e.originalTypePath === "ApiWithAnnotations.query.status",
        ),
      ).toBe(true);
    });
...
export type RealWorldApiData = {
    ...
    url: '/api/resources/{resourceId}/items/{itemId}';
      ...
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "name,date,priority" &&
            e.originalTypePath === "RealWorldApiData.query.sortBy",
        ),
      ).toBe(true);
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "asc,desc" &&
            e.originalTypePath === "RealWorldApiData.query.sortOrder",
        ),
      ).toBe(true);
    });
...
export type GetApiResourcesData = {
    ...
    url: '/api/resources/{resourceId}/items';
      ...
      expect(
        result.some(
          (e) =>
            e.values.join(",") === "activityCount,createdAt,updatedAt" &&
            e.originalTypePath === "GetApiResourcesData.query.sortBy",
        ),
      ).toBe(true);
      expect(
