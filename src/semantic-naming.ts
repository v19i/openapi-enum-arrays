...
export class SemanticNaming {
  ...
  private extractDomainFromOpenApiPath(originalTypePath?: string): string | null {
    if (!originalTypePath) return null

    const openApiMatch = originalTypePath.match(/^(Get|Post|Put|Delete|Patch)V\d+.*?([A-Z][a-z]+(?:[A-Z][a-z]*)*)/g)
    if (openApiMatch) {
      const resourceMatch = originalTypePath.match(/([A-Z][a-z]+(?:[A-Z][a-z]*)*)/g)
      if (resourceMatch) {
        const meaningfulNames = resourceMatch.filter(
          (name) => !['Data', 'Response', 'Request', 'Query', 'Body', 'Params'].includes(name),
        )
  ...
  private extractContext(originalTypePath?: string): string | null {
    ...
    const openApiMatch = originalTypePath.match(/^(Get|Post|Put|Delete|Patch)V\d+(\w+)Data\.(.+)\.(\w+)$/)
    if (openApiMatch) {
  ...
  private generateFromValues(values: string[]): string {
    ...
    const suffix = values.length > 1 ? values[1]?.toLowerCase().substring(0, 2) || '' : ''
