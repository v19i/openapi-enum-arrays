...
class PropertyDetector {
  ...
  extractUnionTypeProperty(
    line: string,
  ): { propertyName: string; typeDefinition: string } | null {
...
export class EnumParser {
  private readonly unionTypeRegex =
    /(['"`])[^'"`]*\1(\s*\|\s*(['"`])[^'"`]*\3)+/g;
  ...
  private parseStandaloneTypes(content: string): EnumInfo[] {
    ...
    while (match !== null) {
      ...
      if (enumValues.length > 0) {
        ...
        const semanticName = this.semanticNaming.generateName(
          enumValues,
          originalTypePath,
        );
  ...
  private extractEnumsFromNestedTypeProperties(content: string): EnumInfo[] {
    ...
    for (const rawLine of sourceLines) {
      ...
      const typeDefinitionName =
        this.propertyDetector.extractTypeDefinitionName(cleanLine);
      ...
      const unionTypeProperty =
        this.propertyDetector.extractUnionTypeProperty(cleanLine);
      if (unionTypeProperty && scopeTracker.hasContext()) {
        const { propertyName, typeDefinition } = unionTypeProperty;

        const hasObjectBrace =
          this.propertyDetector.hasObjectOpeningBrace(cleanLine);
        if (hasObjectBrace) {
        ...
        const cleanTypeDefinition = typeDefinition.replace(/[;,]$/, "").trim();
        const enumValues =
          this.unionExtractor.extractEnumValues(cleanTypeDefinition);

        if (enumValues.length > 0) {
          const currentPath = scopeTracker.getCurrentPath();
          const fullPropertyPath = [...currentPath, propertyName].join(".");
          const enumName = this.semanticNaming.generateName(
            enumValues,
            fullPropertyPath,
          );
